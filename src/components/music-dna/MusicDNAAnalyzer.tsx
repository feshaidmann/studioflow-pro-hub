    ...
    <div className="space-y-4">
      ...
      <section id="dna-referencias" className="scroll-mt-16 space-y-4">
        <DiagCard icon="🔗" title="Referências mais próximas">
          {(() => {
            // Limiar mínimo para considerar um vizinho confiável (similarity_score)
            const MIN_SIM = 0.40;
            const filteredNeighbors = (catalogNeighbors ?? []).filter(
              (n) => Number(n.similarity_score) >= MIN_SIM,
            );
            const catalogBandsLower = new Set(
              (catalogNeighbors ?? [])
                .map((n) => n.band?.toLowerCase().trim())
                .filter(Boolean) as string[],
            );

            // Categoriza similaridade em rótulos
            const categorize = (sim: number) => {
              if (sim >= 0.75) return { label: "Próximo", className: "bg-emerald-500/10 text-emerald-700 border-emerald-500/30" };
              if (sim >= 0.55) return { label: "Aproximado", className: "bg-amber-500/10 text-amber-700 border-amber-500/30" };
              return { label: "Distante", className: "bg-muted text-foreground/70 border-border" };
            };

            // BPM half/double aware: detecta se o melhor match é em metade/dobro
            const bpmRelation = (refBpm: number | null | undefined, userBpm: number | null | undefined) => {
              if (typeof refBpm !== "number" || typeof userBpm !== "number" || !refBpm || !userBpm) return null;
              const dDirect = Math.abs(refBpm - userBpm);
              const dDouble = Math.abs(refBpm - userBpm * 2);
              const dHalf = Math.abs(refBpm - userBpm / 2);
              const min = Math.min(dDirect, dDouble, dHalf);
              if (min === dDouble && dDouble < dDirect - 1) return "double-time";
              if (min === dHalf && dHalf < dDirect - 1) return "half-time";
              return null;
            };

            const topSimDefault = filteredNeighbors.length > 0
              ? Math.round((Number(filteredNeighbors[0].similarity_score) || 0) * 100)
              : 0;
            const hasIa = (referencias_proximas?.length ?? 0) > 0;
            const defaultTab = (filteredNeighbors.length > 0 && (topSimDefault >= 55 || !hasIa))
              ? "catalogo"
              : "ia";

            // Cobertura honesta do catálogo
            const coverageLine = catalogTotal > 0
              ? `Catálogo: ${catalogTotal.toLocaleString("pt-BR")} faixas${
                  diagnosis.genero_classificado && catalogGenreCount > 0
                    ? ` · ${catalogGenreCount} no gênero detectado${strictGenreUsed ? " (filtro ativo)" : ""}`
                    : ""
                }`
              : null;
            ...
            return (
              <>
                {coverageLine && (
                  <p className="mb-2 text-xs text-foreground/60">{coverageLine}</p>
                )}
                <Tabs defaultValue={defaultTab} className="w-full">
                  <TabsList className="grid w-full grid-cols-2 h-9">
                    <TabsTrigger value="catalogo" className="text-xs gap-1">
                      ...
                      {filteredNeighbors.length > 0 && (
                        <span className="text-foreground/60">({filteredNeighbors.length})</span>
                      )}
                      <TooltipProvider delayDuration={150}>
                        <Tooltip>
                          ...
                          <TooltipContent side="bottom" className="max-w-xs text-xs leading-relaxed">
                            Comparação técnica entre extrações (LUFS, dinâmica, espectro, ritmo). Não é fingerprint nem identificação da obra. Vizinhos abaixo de 40% de proximidade são omitidos.
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </TabsTrigger>
                    <TabsTrigger value="ia" className="text-xs gap-1">
                      ...
                      {referencias_proximas && referencias_proximas.length > 0 && (
                        <span className="text-foreground/60">({referencias_proximas.length})</span>
                      )}
                      <TooltipProvider delayDuration={150}>
                        <Tooltip>
                          <TooltipTrigger asChild>
                          ...
                          <TooltipContent side="bottom" className="max-w-xs text-xs leading-relaxed">
                            Sem medição direta — sugestões baseadas em padrões estilísticos do gênero e nos vizinhos do catálogo. Não há percentual real de similaridade.
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </TabsTrigger>
                  ...
                  <TabsContent value="catalogo" className="space-y-2 mt-3">
                    {filteredNeighbors.length === 0 && (
                      <div className="rounded-md border border-border/60 bg-muted/30 p-3 text-xs text-foreground/75 leading-relaxed">
                        Nenhuma faixa do catálogo ficou tecnicamente próxima desta. Isso costuma acontecer quando o gênero da faixa tem cobertura limitada no catálogo de referência. Confira a aba <strong>Sugestões IA</strong> para alternativas estilísticas.
                      </div>
                    )}
                    {filteredNeighbors.map((n, i) => {
                      const sim = Math.round((Number(n.similarity_score) || 0) * 100);
                      const cat = categorize(Number(n.similarity_score) || 0);
                      const dimsUsed = n.dims_used ?? null;
                      const dimsTotal = n.dims_total ?? 15;
                      const bpmRel = bpmRelation(n.tempo_bpm, realAnalysis?.bpm);
                      return (
                        <button
                          type="button"
                          ...
                          className={cn(
                            "w-full text-left flex flex-col gap-1.5 py-2.5 px-2 -mx-2 rounded-md text-xs transition-colors hover:bg-muted/40",
                            i < filteredNeighbors.length - 1 && "border-b border-border",
                          )}
                        >
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <div className="flex flex-col items-end shrink-0 gap-0.5">
                              <span className={cn("inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-medium", cat.className)}>
                                {cat.label}
                                <span className="font-mono opacity-70">({sim}%)</span>
                              </span>
                              {n.genre && (
                                <span className="text-[10px] text-foreground/60 uppercase tracking-wide">{n.genre}</span>
                              )}
                            </div>
                          </div>
                          <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs font-mono text-foreground/70">
                            <span>
                              BPM {fmt(n.tempo_bpm, 0)}
                              {bpmRel ? (
                                <span className="text-amber-700/80"> ({bpmRel})</span>
                              ) : (
                                <span className="text-primary/70"> ({fmtDelta(n.tempo_bpm, realAnalysis?.bpm, 0)})</span>
                              )}
                            </span>
                            ...
                            {dimsUsed != null && (
                              <span className="text-foreground/55" title="Dimensões efetivamente comparadas">
                                {dimsUsed}/{dimsTotal} dims
                              </span>
                            )}
                          </div>
                        </button>
                  ...
                  <TabsContent value="ia" className="space-y-2 mt-3">
                    <p className="rounded-md bg-muted/30 p-2.5 text-xs text-foreground/75 leading-relaxed">
                      Sugestões estilísticas da IA — não são medições e não devem ser lidas como "sua música é parecida com X". Use como inspiração de território sonoro.
                    </p>
                    {(!referencias_proximas || referencias_proximas.length === 0) && (
                      <p className="text-xs text-foreground/70 py-3 text-center">Nenhuma sugestão estilística disponível.</p>
                    )}
                    {(referencias_proximas ?? []).map((r, i) => {
                      const inCatalog = catalogBandsLower.has((r.artista ?? "").toLowerCase().trim());
                      return (
                        <div key={i} className={cn(
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2 flex-wrap">
                              ...
                              <span className={cn(
                                ...
                                {inCatalog ? "vizinho do catálogo" : "biblioteca de territórios"}
                              </span>
                            </div>
