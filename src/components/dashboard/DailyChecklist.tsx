                            ...
                            {aiRef?.current && (
                              <Button
                                ...
                                onClick={() => {
                                  ...
                                  if (projectName) ctxBits.push(`Projeto: ${projectName}`);
                                  if (task.assignedTo) ctxBits.push(`Responsável: ${task.assignedTo}`);
                                  if (task.dueDate) ctxBits.push(`Prazo: ${task.dueDate}`);
                                  if (task.severity && task.severity !== "medium") ctxBits.push(`Severidade: ${task.severity}`);
                                  if (task.blocked && task.blockedReason) ctxBits.push(`Bloqueio: ${task.blockedReason}`);
                                  if (task.taskArea && task.taskArea !== "geral") ctxBits.push(`Área: ${task.taskArea}`);
                                  const ctxLine = ctxBits.length ? `\n(${ctxBits.join(" · ")})` : "";

                                  let prompt: string;
                                  if (task.source === "music-dna" || task.description.startsWith("[DNA]")) {
                                    prompt = `Me explique de forma prática como aplicar esta técnica: ${desc}${ctxLine}`;
                                  } else if (task.blocked) {
                                    prompt = `Esta tarefa está bloqueada: "${desc}".${ctxLine}\nMe ajude a destravar: o que posso fazer agora, quem acionar e como reduzir a dependência?`;
                                  } else if (task.autoGenerated) {
                                    prompt = `Me ajude com esta tarefa automática (origem: ${sourceMeta.label}): "${desc}".${ctxLine}\nExplique por que ela apareceu, qual o impacto se eu ignorar e qual o passo prático para resolver agora.`;
                                  } else {
                                    prompt = `Me ajude a executar esta tarefa: "${desc}".${ctxLine}\nQuebre em passos práticos, sugira ferramentas/contatos e estime quanto tempo deve levar.`;
                                  }
                                  aiRef.current?.sendMessage(prompt);
                                  onInvokeAI?.();
                                }}
                              >
