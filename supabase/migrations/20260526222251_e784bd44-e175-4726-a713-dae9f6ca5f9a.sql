-- 1) Quarentenar as 3 faixas sem feature alguma
UPDATE public.music_reference_tracks
   SET quarantined = true,
       quarantine_reason = COALESCE(NULLIF(quarantine_reason,''), 'no_features'),
       updated_at = now()
 WHERE quarantined = false
   AND mfcc IS NULL
   AND lufs_integrated IS NULL
   AND spectral_centroid IS NULL;

-- 2) Atualizar a RPC com filtro mínimo de qualidade no candidate set
CREATE OR REPLACE FUNCTION public.find_nearest_reference_tracks(
  ...
  p_mfcc double precision[] DEFAULT NULL::double precision[],
  p_chroma_cens double precision[] DEFAULT NULL::double precision[],
 ...
 RETURNS TABLE(band text, filename text, genre text, tempo_bpm numeric, key_name text, mode text, lufs_integrated numeric, dynamic_range_db numeric, energy numeric, danceability numeric, valence numeric, acousticness numeric, instrumentalness numeric, spectral_centroid numeric, speechiness numeric, liveness numeric, spectral_flatness numeric, zero_crossing_rate numeric, similarity_score numeric)
 LANGUAGE sql
