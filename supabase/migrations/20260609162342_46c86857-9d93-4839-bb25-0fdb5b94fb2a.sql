...
-- Permitir summary_variant versionado (ex.: 'A.v2', 'B.v2') sem perder o histórico 'A'/'B'.
-- Mantém validação leve: formato BASE[.vN] onde BASE ∈ {A,B}.

ALTER TABLE public.music_dna_analyses
  DROP CONSTRAINT IF EXISTS music_dna_analyses_summary_variant_check;
ALTER TABLE public.music_dna_analyses
  ADD CONSTRAINT music_dna_analyses_summary_variant_check
  CHECK (summary_variant ~ '^[AB](\.v[0-9]+)?$');

ALTER TABLE public.diagnosis_acceptance_signals
  DROP CONSTRAINT IF EXISTS diagnosis_acceptance_signals_summary_variant_check;
ALTER TABLE public.diagnosis_acceptance_signals
  ADD CONSTRAINT diagnosis_acceptance_signals_summary_variant_check
  CHECK (summary_variant ~ '^[AB](\.v[0-9]+)?$');

-- Atualiza a RPC para considerar qualquer variante versionada e agrupar pelo rótulo completo.
  ...
  WITH base AS (
    SELECT a.id, a.summary_variant
     ...
     WHERE a.legacy = false
       AND a.summary_variant ~ '^[AB](\.v[0-9]+)?$'
  ),
  per_analysis AS (
